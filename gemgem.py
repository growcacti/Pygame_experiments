    ...
    while True:  # main game loop
        ...
        for event in pygame.event.get():  # event handling loop
            if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
                pygame.display.quit()
                pygame.quit()  # Display quit is a workaround for a Raspberry Pi bug.
                sys.exit()
                ...
                if event.pos == (lastMouseDownX, lastMouseDownY):
                    ...
                    firstSelectedGem = checkForGemClick(
        ...
        if clickedSpace and not firstSelectedGem:
            ...
            firstSwappingGem, secondSwappingGem = getSwappingGems(
                gameBoard, firstSelectedGem, clickedSpace
            )
            ...
            boardCopy = getBoardCopyMinusGems(
                gameBoard, (firstSwappingGem, secondSwappingGem)
            )
            animateMovingGems(
                boardCopy, [firstSwappingGem, secondSwappingGem], [], score
            )

            # Swap the gems in the board data structure.
            gameBoard[firstSwappingGem["x"]][firstSwappingGem["y"]] = secondSwappingGem[
                "imageNum"
            ]
            gameBoard[secondSwappingGem["x"]][
                secondSwappingGem["y"]
        ...
        if gameIsOver:
            if clickContinueTextSurf == None:
                ...
                clickContinueTextRect.center = int(WINDOWWIDTH / 2), int(
                    WINDOWHEIGHT / 2
                )
...
def animateMovingGems(board, gems, pointsText, score):
    ...
    while progress < 100:  # animation loop
        ...
        FPSCLOCK.tick(FPS)
        progress += (
            MOVERATE  # progress the animation a little bit more for the next frame
        )
