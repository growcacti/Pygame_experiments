...
class Scene(object):
    ...
    def __init__(self, size=None, max_ups=None, max_fps=None):
        ...
        spyral.event.register(
            "director.update", self._run_actors, ("delta",), scene=self
        )
        spyral.event.register(
            "spyral.internal.view.changed", self._invalidate_views, scene=self
        )
    ...
    def _send_event_to_handler(
        self, event, type, handler, args, kwargs, priority, dynamic
    ):
        """
        ...
        def _get_arg_val(arg, default=fillval):
            if arg == "event":
                ...
                if default != fillval:
                    return default
                raise TypeError(
                    "Handler expects an argument named "
                    "%s, %s does not have that." % (arg, str(type))
                )
        ...
        if handler is sys.exit and args is None and kwargs is None:
            ...
            args = [
                _get_arg_val(arg, default) for arg, default in zip(h_args, h_defaults)
            ]
            kwargs = {}
        elif args is None:
            args = []
    ...
    def _handle_event(self, type, event=None):
        ...
        handlers = chain.from_iterable(
            self._handlers[namespace] for namespace in self._get_namespaces(type)
        )
    ...
    def _unregister_sprite_events(self, sprite):
        for name, handlers in self._handlers.items():
            self._handlers[name] = [
                ...
                for h in handlers
                if (
                    not isinstance(h[0], WeakMethodBound)
    ...
    def _unregister(self, event_namespace, handler):
        ...
        self._handlers[event_namespace] = [
            h
            for h in self._handlers[event_namespace]
            if (
                (not isinstance(h[0], WeakMethodBound) and handler != h[0])
                or (
                    isinstance(h[0], WeakMethodBound)
                    and (
                        (h[0].func is not handler.im_func)
    ...
    def _apply_style(self, obj):
        ...
        if not hasattr(obj, "__stylize__"):
            raise spyral.NotStylableError(
                ("%r is not an object" "which can be styled.") % obj
            )
        properties = {}
    ...
    def _set_size(self, size):
        ...
        self._size = size
        self._scale = (rsize[0] / size[0], rsize[1] / size[1])
    ...
    def _set_background(self, image):
        ...
        if surface.get_size() != self.size:
            raise spyral.BackgroundSizeError(
                "Background size must match " "the scene's size."
            )
    ...
    def _draw(self):
        ...
        for blit in blits:
            ...
            if not screen_rect.contains(blit_rect) and not screen_rect.colliderect(
                blit_rect
            ):
                continue
            if blit.static:
    ...
    def _set_layers(self, layers):
        ...
        if self._layers == []:
            ...
            raise spyral.LayersAlreadySetError(
                "You can only define the layers " "for a scene once."
            )
