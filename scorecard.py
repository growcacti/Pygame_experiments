...
class Scorecard(object):
    ...
    def make_in_out_labels(self, player):
        ...
        current_front_par = sum(
            (HOLE_INFO[x]["par"] for x in range(1, min(10, player.hole_num + 1)))
        )
        current_front_score = sum(
            (player.scores[x] for x in range(1, min(10, player.hole_num + 1)))
        )
        ...
        if player.hole_num < 10:
            ...
            current_back_par = sum(
                (HOLE_INFO[x]["par"] for x in range(10, player.hole_num + 1))
            )
            current_back_score = sum(
                (player.scores[x] for x in range(10, player.hole_num + 1))
            )
            c_back_color = "gray20"
        ...
        current_total_par = sum(
            (HOLE_INFO[x]["par"] for x in range(1, player.hole_num + 1))
        )
        ...
        Label(
            "{}".format(total_par),
            {"center": self.total_par_center},
            self.labels,
            text_color="goldenrod",
            **self.style
        )
        Label(
            "{}".format(current_front_score),
            {"center": self.current_out_center},
            self.labels,
            text_color=c_front_color,
            **self.style
        )
        Label(
            "{}".format(current_back_score),
            {"center": self.current_in_center},
            self.labels,
            text_color=c_back_color,
            **self.style
        )
        Label(
            "{}".format(current_total_score),
            {"center": self.current_total_center},
            self.labels,
            text_color=c_total_color,
            **self.style
        )

    def make_hole_labels(self, player):
        for h, hole_center in zip(
            range(1, len(self.hole_centers) + 1), self.hole_centers
        ):
            Label(
                "{}".format(h),
                {"center": hole_center},
    ...
    def make_best_score_labels(self, player):
        ...
        Label(
            "{}".format(front_score),
            {"center": self.best_score_out_center},
            self.labels,
            text_color=front_score_color,
            **self.style
        )
        Label(
            "{}".format(back_score),
            {"center": self.best_score_in_center},
            self.labels,
            text_color=back_score_color,
            **self.style
        )
        Label(
            "{}".format(total_score),
            {"center": self.best_score_total_center},
            self.labels,
            text_color=total_score_color,
            **self.style
        )
        for hole_num, best_score in sorted(player.best_scores.items()):
            ...
            Label(
                "{}".format(score),
                {"center": self.best_score_centers[hole_num - 1]},
                self.labels,
                text_color=color,
                **self.style
            )
    ...
    def make_best_round_labels(self, player):
        ...
        Label(
            "{}".format(front_score),
            {"center": self.best_round_out_center},
            self.labels,
            text_color=front_score_color,
            **self.style
        )
        Label(
            "{}".format(back_score),
            {"center": self.best_round_in_center},
            self.labels,
            text_color=back_score_color,
            **self.style
        )
        Label(
            "{}".format(total_score),
            {"center": self.best_round_total_center},
            self.labels,
            text_color=total_score_color,
            **self.style
        )
        for hole_num, best_score in sorted(player.best_round.items()):
            ...
            Label(
                "{}".format(score),
                {"center": self.best_round_centers[hole_num - 1]},
                self.labels,
                text_color=color,
                **self.style
            )
