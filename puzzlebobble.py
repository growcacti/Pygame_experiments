...
class Bubble(pygame.sprite.Sprite):
    ...
    def draw(self):
        pygame.gfxdraw.filled_circle(
            DISPLAYSURF, self.rect.centerx, self.rect.centery, self.radius, self.color
        )
        pygame.gfxdraw.aacircle(
            DISPLAYSURF, self.rect.centerx, self.rect.centery, self.radius, GRAY
        )
...
def runGame():
    ...
    while True:
        ...
        if launchBubble == True:
            ...
            launchBubble, newBubble, score = stopBubble(
                bubbleArray, newBubble, launchBubble, score
            )
            ...
            for row in range(len(bubbleArray)):
                for column in range(len(bubbleArray[0])):
                    if bubbleArray[row][column] != BLANK:
                        ...
                        if bubbleArray[row][column].rect.bottom > (
                            WINDOWHEIGHT - arrow.rect.height - 10
                        ):
...
def popFloaters(bubbleArray, copyOfBoard, column, row=0):
    if (
        row < 0
        or row > (len(bubbleArray) - 1)
        or column < 0
        or column > (len(bubbleArray[0]) - 1)
    ):
        return
    ...
    for row in range(len(bubbleArray)):
        for column in range(len(bubbleArray[row])):
            ...
            if bubbleArray[row][column] != BLANK and newBubble != None:
                if (
                    pygame.sprite.collide_rect(newBubble, bubbleArray[row][column])
                ) or newBubble.rect.top < 0:
                    if newBubble.rect.top < 0:
                        newRow, newColumn = addBubbleToTop(bubbleArray, newBubble)

                    elif (
                        newBubble.rect.centery >= bubbleArray[row][column].rect.centery
                    ):
                    ...
                    popBubbles(
                        bubbleArray, newRow, newColumn, newBubble.color, deleteList
                    )
