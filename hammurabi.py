...
while True:  # Main game loop.
    ...
    print(
        "        Land: {} acres ({} per person)".format(
            acresOwned, acresOwned // population
        )
    )
    print(
        "     Harvest: {} bushels per acres, {} total".format(
            bushelsHarvestedPerAcre, bushelsHarvestedPerAcre * acresToPlant
        )
    )
    ...
    print(
        "Land trades between 17 & 26 bushels per acre. Current price: {}".format(
            landPricePerAcre
        )
    )
    ...
    while True:
        print(
            "How many acres do you wish to buy? (0-{})".format(
                bushelsStored // landPricePerAcre
            )
        )
        try:
        ...
        print(
            "Spent {} bushels to buy {} acres. Current bushels: {} Current land: {}".format(
                acresToBuy * landPricePerAcre, acresToBuy, bushelsStored, acresOwned
            )
        )
        break
    ...
    if acresToBuy == 0:  # Only ask to sell land if they didn't buy land.
        while True:
            ...
            print(
                "Sold {} acres for {} bushels. Current bushels: {} Current land: {}".format(
                    acresToSell,
                    acresToSell * landPricePerAcre,
                    bushelsStored,
                    acresOwned,
                )
            )
    ...
    while True:
        ...
        print(
            "20 bushels feeds 1 person: How many bushels do you wish to feed your people? (0-{})".format(
                maxToFeed
            )
        )
        try:
        ...
        print(
            "Fed {} people with {} bushels. Current bushels: {}".format(
                bushelsToFeed // 20, bushelsToFeed, bushelsStored
            )
        )
        break
    ...
    while True:
        ...
        print(
            "2 bushels to plant 1 acre, 1 person can plant 10 acres: How many acres do you wish to plant with seed? (0-{})".format(
                maxCanPlant
            )
        )
        try:
        ...
        print(
            "Planted {} acres using {} bushels. Current bushels: {}".format(
                acresToPlant, acresToPlant * 2, bushelsStored
            )
        )
        break
    ...
    # Calculate births:
    births = int(
        random.randint(1, 5) * (20 * acresOwned + bushelsStored) / population / 100 + 1
    )
...
percentageDiedPerYear = (
    sum(percentageDiedEachYear) // 10
)  # TODO This is wrong and needs to be corrected. Get the percentage deaths each year, and find the average of that. See original source.
...
if percentageDiedPerYear > 33 or acresPerPerson < 7:
    ...
    print(
        "really wasn't too bad at all. {} people".format(
            random.randint(0, int(population * 0.8))
        )
    )
