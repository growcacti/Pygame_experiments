...
class Viewer:
    ...
    def setup_screen(self, width, height, backgroundcolor=(255, 255, 255)):
        ...
        Viewer.screen = pygame.display.set_mode((width, height), pygame.DOUBLEBUF)
    ...
    def create_menu(self):
        ...
        colors = [
            str(hex(r))[-2:].replace("x", "0")
            + str(hex(g))[-2:].replace("x", "0")
            + str(hex(b))[-2:].replace("x", "0")
        ...
        colormenu = Menu(
            ...
            items=[
                Item(
                    ...
                    choices=colors,
                    cindex=-2,
                    helptext="hexadecimal values for red, green, blue. (00=0, ff=255)",
                ),
                Item(
                    "color_small_font1",
                    choices=colors,
        ...
        settingsmenu = Menu(
            name="settings", items=[audiomenu, videomenu, fontsizemenu, colormenu]
        )
        # ---- merge all submenus into root menu ------
        rootmenu = Menu(
            "root", [Item("play"), Item("credits"), settingsmenu, Item("quit")]
        )
            ...
            for k, v in menuvalues_new.items():
                ...
                if menuvalues_old[k] != v:
                    ...
                    if k == "color_background":
                        ...
                        Viewer.menu1.background.fill(
                            pygame.Color(
                                int(v[0:2], 16), int(v[2:4], 16), int(v[4:6], 16)
                            )
                        )
                    if k == "color_small_font1":
                        Viewer.menu1.helptextcolor1 = pygame.Color(
                            int(v[0:2], 16), int(v[2:4], 16), int(v[4:6], 16)
                        )
                    if k == "color_small_font2":
                        Viewer.menu1.helptextcolor2 = pygame.Color(
                            int(v[0:2], 16), int(v[2:4], 16), int(v[4:6], 16)
                        )
                    if k == "color_big_font":
                        Viewer.menu1.textcolor = pygame.Color(
                            int(v[0:2], 16), int(v[2:4], 16), int(v[4:6], 16)
                        )
                    if k == "fontsize_small":
...
class PygameMenu:
    ...
    def font(self):
        ...
        return pygame.font.SysFont(
            name=self.fontname, size=self.fontsize, bold=True, italic=False
        )
    ...
    def smallfont(self):
        ...
        return pygame.font.SysFont(
            name=self.fontname, size=self.helptextfontsize, bold=True, italic=False
        )
    ...
    def cursor_back(self):
        ...
        if len(self.history) == 0:
            ...
            print(
                "you are already at root menu... going back is not possible from here"
            )
            return
        ...
        for name in self.history:
            for item in self.menu.items:
                ...
                raise ValueError(
                    f"could not find history item {name} in menuitems {self.menu.items}"
                )
    ...
    def cursor_goto_menu(self, targetname, menu):
        ...
        for item in menu.items:
            print(
                "searching", targetname, " checking item:", item, "in menu:", menu.name
            )
            if type(item) == Menu:
    ...
    def cursor_goto_submenu(self, name):
        ...
        if name not in [item.name for item in self.menu.items if type(item) == Menu]:
            raise ValueError(
                f"no submenu named {name} in current Menuitems: {self.menu.items}"
            )
    ...
    def run(self):
        ...
        while running:
            ...
            for i, entry in enumerate(menupoints):
                if i == self.i:
                    ...
                    write(
                        self.screen,
                        cursortext,
                        cx - maxcursordistance + cursordistance,
                        cy + dy * i,
                        cursorcolor,
                        self.font,
                        origin="topright",
            ...
            t = "press \u2191 \u2193 to navigate, ESC to quit {}".format(
                "\u21D0 for previous menu " if self.menu.name != "root" else ""
            )
            w, h = write(
                self.screen,
                t,
            ...
            if type(activeitem) == Menu:
                ...
                if len(activeitem.choices) <= 1:
                    ...
                    w, h2 = write(
                        self.screen,
                        "press \u2190 \u2192 /Mousewheel/PgUp/PgDown to select values, ENTER/Leftclick to accept",
                        helpx,
                        helpy + h,
                        self.helptextcolor1,
                        self.smallfont,
            ...
            if type(activeitem) == Item and len(activeitem.choices) > 1:
                ...
                for ctext in activeitem.choices:
                    if activeitem.name[0:9] == "fontsize_":
                        font = pygame.font.SysFont(
                            name=self.fontname, size=int(ctext), bold=True, italic=False
                        )
                    else:
                        font = self.smallfont
                    w, h = font.size(ctext)
                ...
                choices_surface.fill(
                    (255, 255, 255)
                )  # choices_surface always has a white background
                ...
                y1 = (
                    self.menu.items[self.i].rect.y
                    + self.menu.items[self.i].rect.height // 2
                )
                self.screen.blit(
                    choices_surface,
                    (
                        Viewer.width - max_w - 50,
                        y1
                        - choicerects[0].height // 2
                        - choicerects[activeitem.cindex].y,
                    ),
                )
