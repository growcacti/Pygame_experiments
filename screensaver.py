...
def screensaver(screenresolution=(640, 480), fullscreen=False):
    ...
    if fullscreen:
        screen = pygame.display.set_mode(
            (screenresolution[0], screenresolution[1]), pygame.FULLSCREEN
    ...
    while mainloop:
        ...
        pygame.draw.circle(
            ...
            color,
            (
                random.randint(0, screenresolution[0]),
        ...
        pygame.display.set_caption(
            "press ESC to quit. FPS: %.2f (%ix%i), time: %.2f seonds"
            % (clock.get_fps(), screenresolution[0], screenresolution[1], playtime)
        )
