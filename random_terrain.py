...
class Game:
    ...
    scene1 = v.canvas(
        title=f"landscape world {world_size} x {world_size}, roughness {roughness}",
        width=1800,
        height=700,
...
def func_roughness(s):
    ...
    Game.label_roughness.text = (
        f" ={Game.roughness:.2f} (0 = very smooth, 1= very rough)"
    )
    # try:
...
def create_widgets():
    ...
    Game.slider_roughness = v.slider(
        bind=func_roughness, min=0, max=1.0, step=0.01, value=Game.roughness, length=500
    )
    ...
    Game.input_sea_level = v.winput(
        bind=func_sea_level, type="numeric", text=Game.sea_level
    )
    ...
    Game.input_snow_level = v.winput(
        bind=func_snow_level, type="numeric", text=Game.snow_level
    )
    ...
    Game.input_world_size = v.winput(
        bind=func_world_size, type="numeric", text=Game.world_size
    )
...
def recalc_y_values():
    ...
    cells = len(
        range(0, Game.world_size + 1, Game.grid_size)
    )  # +1 is important because quads need neighbors
    ...
    for z, line in enumerate(map1):
        ...
        for x, y_value in enumerate(line):
            pointline.append(
                v.vertex(pos=v.vec(x, y_value, z), color=get_color_from_y(y_value))
            )
...
def create_grid():
    ...
    for z in range(0, Game.world_size + 1, Game.grid_size):
        Game.grid.append(
            v.curve(v.vector(0, 0, z), v.vector(Game.world_size, 0, z), color=basecolor)
        )
    for x in range(0, Game.world_size + 1, Game.grid_size):
        Game.grid.append(
            v.curve(v.vector(x, 0, 0), v.vector(x, 0, Game.world_size), color=basecolor)
        )
    # xy wall
    starty = 0 if Game.min_y >= 0 else int(Game.min_y) - 1
    endy = int(Game.max_y) + 1 if Game.max_y > 0 else 0
    wall1 = v.box(
        pos=v.vector(Game.world_size / 2, starty + (endy - starty) / 2, 0),
    ...
    for y in range(starty, endy, Game.grid_size):
        Game.grid.append(
            v.curve(v.vector(0, y, 0), v.vector(Game.world_size, y, 0), color=basecolor)
        )
    for x in range(0, Game.world_size + 1, Game.grid_size):
        Game.grid.append(
            v.curve(v.vector(x, starty, 0), v.vector(x, endy, 0), color=basecolor)
        )
    # zy wall
    wall2 = v.box(
        pos=v.vector(0, starty + (endy - starty) / 2, Game.world_size / 2),
    ...
    for y in range(starty, endy, Game.grid_size):
        Game.grid.append(
            v.curve(v.vector(0, y, 0), v.vector(0, y, Game.world_size), color=basecolor)
        )
    for z in range(0, Game.world_size + 1, Game.grid_size):
        Game.grid.append(
            v.curve(v.vector(0, starty, z), v.vector(0, endy, z), color=basecolor)
        )
    # curves CANNOT be part of compound
    max_y_box = v.box(
        pos=v.vector(0, Game.max_y, 0),
        color=v.vector(1, 1, 0),
        size=v.vector(1, 0.1, 1),
    )
    min_y_box = v.box(
        pos=v.vector(0, Game.min_y, 0),
        color=v.vector(0, 1, 1),
        size=v.vector(1, 0.1, 1),
    )
    Game.worldbox = v.compound(
...
def create_world():
    ...
    xarrow = v.arrow(
        pos=v.vector(0, 0, 0), axis=v.vector(1, 0, 0), color=v.vector(1, 0, 0)
    )
    xletter = v.text(
        pos=xarrow.axis,
        text="x",
    ...
    yarrow = v.arrow(
        pos=v.vector(0, 0, 0), axis=v.vector(0, 1, 0), color=v.vector(0, 1, 0)
    )
    yletter = v.text(
        pos=yarrow.axis,
        text="y",
    ...
    zarrow = v.arrow(
        pos=v.vector(0, 0, 0), axis=v.vector(0, 0, 2), color=v.vector(0, 0, 1)
    )
    zletter = v.text(
        pos=zarrow.axis,
        text="z",
