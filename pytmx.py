    ...
    def set_properties(self, node):
        ...
        for k, v in parse_properties(node).items():
            if k in self.reserved:
                ...
                print msg.format(
                    self.__class__.__name__, self.name, k, self.__class__.__name__
                )
...
class TiledMap(TiledElement):
    ...
    def getTileLocation(self, gid):
        ...
        p = product(
            xrange(self.width), xrange(self.height), xrange(len(self.tilelayers))
        )

        return [(x, y, l) for (x, y, l) in p if self.tilelayers[l].data[y][x] == gid]
...
class TiledLayer(TiledElement):
    ...
    def parse(self, node):
        ...
        if encoding == "base64":
            ...
            next_gid = imap(
                int, "".join(line.strip() for line in data_node.text.strip()).split(",")
            )
