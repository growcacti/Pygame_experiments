...
class Gameplay(tools._State):
    def __init__(self):
        ...
        self.bg = ScreenBackground(
            prepare.GFX["bg-screen"], [(0, 0, 378, 150), (904, 4, 371, 544)]
        )
        ...
        if event.type == pg.KEYUP:
            ...
            for cell in self.grid.cells.values():
                if cell.rect.collidepoint(event.pos) and cell.jewel is not None:
                    ...
                    self.grab_point = (
                        event.pos[0] - self.grabbed.jewel.rect.left,
            ...
            if dx >= dy:
                if jewel.rect.left < gr.left:
                    if neighbors["left"] is not None:
                        jewel.rect.left = max(
                            jewel.rect.left, neighbors["left"].rect.left
                        )
                        self.dest_tile = neighbors["left"]
                        if self.dest_tile.jewel is not None:
                            right = gr.left + (
                                self.dest_tile.rect.right - jewel.rect.left
                            )
                    ...
                    if neighbors["right"] is not None:
                        jewel.rect.right = min(
                            jewel.rect.right, neighbors["right"].rect.right
                        )
                        self.dest_tile = neighbors["right"]
                        if self.dest_tile.jewel is not None:
                            self.dest_tile.jewel.rect.left = gr.right - (
                                jewel.rect.right - self.dest_tile.rect.left
                            )
                    else:
                ...
                if jewel.rect.top < gr.top:
                    if neighbors["up"] is not None:
                        ...
                        if self.dest_tile.jewel is not None:
                            self.dest_tile.jewel.rect.bottom = gr.top - (
                                jewel.rect.top - self.dest_tile.rect.bottom
                            )
                    else:
                        jewel.rect.top = gr.top
                elif jewel.rect.bottom > gr.bottom:
                    if neighbors["down"] is not None:
                        jewel.rect.bottom = min(
                            jewel.rect.bottom, neighbors["down"].rect.bottom
                        )
                        self.dest_tile = neighbors["down"]
                        if self.dest_tile.jewel is not None:
                            self.dest_tile.jewel.rect.top = gr.bottom - (
                                jewel.rect.bottom - self.dest_tile.rect.top
                            )
                    else:
