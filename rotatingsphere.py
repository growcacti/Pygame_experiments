...
def main():
    root = tk.Tk()
    canvas = tk.Canvas(root, width=400, height=200, background="wheat")
    ...
    DEFAULT_SCALEY = (
        HEIGHT - 4
    ) // 2  # Text cells are twice as tall as they are wide, so set scaley accordingly.
    ...
    def transformPoint(
        point, scalex=None, scaley=None, translatex=None, translatey=None
    ):
        if scalex == None:
        ...
        while True:  # Main program loop.
            ...
            for y in range(0, HEIGHT, 2):
                ...
                for x in range(WIDTH):
                    canvas.create_line(
                        WIDTH, HEIGHT, WIDTH + 1, HEIGHT + 1, fill="black", width=2
                    )
                    if (x, y) in spherePoints and (x, y + 1) in spherePoints:
                        ...
                        print(
                            chr(9604), end="", flush=False
                        )  # Draw bottom half of block.
                    else:
                        print(" ", end="", flush=False)  # Draw empty space.
                print()
            print("", end="", flush=True)

            time.sleep(1.55)  # Pause for a bit.
