...
JEWEL_NUMS = [1, 4, 5, 3]
JEWEL_COMBOS = {}
JEWEL_COMBOS = [(color, num) for color in JEWEL_COLORS for num in JEWEL_NUMS]
ANI_SPEED = 4
...
class GridCell(object):
    def __init__(self, topleft, index, size):
        ...
        w, h = size
        self.rect = pg.Rect(
            topleft[0] + (w * index[0]), topleft[1] + (h * index[1]), w, h
        )
        self.jewel = None
...
class TileGrid(object):
    ...
    def __init__(
        self, topleft, num_rows, num_columns, cell_size, color_scheme_num, saved=None
    ):
        ...
        self.rect = pg.Rect(
            topleft, (num_columns * cell_size[0], num_rows * cell_size[1])
        )
        self.reset()
        self.recheck = False
        if saved is None:
            ...
            self.elapsed = 0
            self.max_bonus = 1000
            self.bonus = int(self.max_bonus * 0.5)
            self.bonus_cooldown = 0.01
            self.level = 1
            self.num_combos = 6
        ...
        self.background = BouncingBackground(
            self.rect.topleft,
            (self.rect.w, self.rect.h + 32),
            prepare.GFX["bg-jewels"],
            60,
        )
        self.ui = GridUI(self)
    ...
    def make_cells(self, cell_size):
        self.cells = {
            (x, y): GridCell(self.topleft, (x, y), (cell_size))
    ...
    def fill_jewels(self):
        for cell in self.rows[0]:
            if cell.jewel is None:
                ...
                ani = Animation(
                    top=cell.rect.top, duration=int(dist * ANI_SPEED), round_values=True
                )
    ...
    def find_row_matches(self):
        ...
        for i, row in enumerate(self.rows):
            colors = (
                (cell.jewel.color, cell.jewel.gem_num) if cell.jewel else None
                for cell in row
            )
    ...
    def spin_up(self, num):
        ...
        spin_up = Animation(spin_speed=speeds[num], duration=350, round_values=True)
    ...
    def make_background(self):
        ...
        if int(self.bg_scroll) + self.rect.h > h:
            bottom_rect = pg.Rect(
                0, int(self.bg_scroll), self.rect.w, h - int(self.bg_scroll)
            )
            top_rect = pg.Rect(
                0, 0, self.rect.w, self.rect.h - (h - int(self.bg_scroll))
            )
    ...
    def update(self, dt):
        ...
        if not self.animations:
            ...
            empty_cells = any((cell.jewel is None for cell in self.cells.values()))
            if empty_cells:
                self.fill_jewels()
