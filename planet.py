...
class Planet(pg.sprite.DirtySprite):
    def __init__(self, pos, image, radius, rotations_per_minute, color, *groups):
        ...
        self.make_cover()
        self.rotation_speed = (rotations_per_minute / 60.0) * self.circum / 1000.0
    ...
    def update(self, dt, viewport, planets):
        ...
        self.redraw = False
        self.source_pos = (
            self.source_pos[0] + self.rotation_speed * dt,
            self.source_pos[1],
        )
    ...
    def make_image(self):
        ...
        if rect.right > self.circum:
            left = pg.Rect(
                rect.left, rect.top, self.circum - rect.left, self.radius * 2
            )
