...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, direction, color, type_, win):
        ...
        self.rect = pygame.draw.circle(
            self.win, self.color, (self.x, self.y), self.radius
        )
...
class Grenade(pygame.sprite.Sprite):
    def __init__(self, x, y, direction, win):
        ...
        self.rect = pygame.draw.circle(
            self.win, (255, 50, 50), (self.x, self.y), self.radius
        )
        ...
        if self.speed == 0:
            ...
            if self.timer <= 0:
                ...
                p_distance = math.sqrt(
                    (p.rect.centerx - self.x) ** 2 + (p.rect.centery - self.y) ** 2
                )
                if p_distance <= 100:
                    if p_distance > 80:
                        p.health -= 20
                ...
                for e in enemy_group:
                    e_distance = math.sqrt(
                        (e.rect.centerx - self.x) ** 2 + (e.rect.centery - self.y) ** 2
                    )
                    if e_distance < 80:
                        e.health -= 100
